...
The current Comms follow-on is the DZN Comms message/read model approval preflight in `docs/DZN_COMMS_MESSAGE_READ_MODEL_APPROVAL_PREFLIGHT.md`, after the DZN Comms reaction runtime implementation approval preflight in `docs/DZN_COMMS_REACTION_RUNTIME_IMPLEMENTATION_APPROVAL_PREFLIGHT.md` confirmed that message/read authorization is required before runtime reactions. It still builds on the DZN Comms reaction interaction contract preflight in `docs/DZN_COMMS_REACTION_INTERACTION_CONTRACT_PREFLIGHT.md`. The message/read preflight defines public-channel reads, private group membership proof, message visibility states, no-store responses, and the mock-to-real transition before any runtime message-history route, message table, reaction route, reaction table, Durable Object, WebSocket, persistence, analytics/tracking, AI provider, vector store, metered model call, live checkout, production mutation, or issue #49 change.
