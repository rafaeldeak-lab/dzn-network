...
assert.equal(serverMetadata.includes("/gameservers/stats"), true, "Cron metadata refresh should prefer Nitrado's server-side getStats-equivalent API when available.");
assert.equal(serverMetadata.includes("gameservers_stats"), true, "Cron metadata refresh should record API stats as the source when selected.");
assert.equal(serverMetadata.includes("/wi/gameserver/ajax/getStats"), true, "Cron metadata refresh should also try webinterface getStats when available server-side.");
