        ...
        <aside className="grid content-start gap-5">
          <section className="rounded-lg border border-white/10 bg-white/[0.045] p-4">
            ...
            <p className="mt-3 text-sm font-bold leading-6 text-zinc-400">
              These controls change this preview only. Public profile publishing stays off until a later profile settings slice adds saved privacy preferences.
            </p>
            ...
            <div className="mt-4 grid gap-2">
              <PrivacyToggle label="Show XP" checked={showXp} onChange={setShowXp} disabled={hiddenMode} />
              <PrivacyToggle label="Show challenge progress" checked={showChallenges} onChange={setShowChallenges} disabled={hiddenMode} />
              <PrivacyToggle label="Show calling cards" checked={showCards} onChange={setShowCards} disabled={hiddenMode} />
            </div>
            <div className="mt-4 rounded border border-emerald-300/20 bg-emerald-400/10 p-3 text-xs font-bold leading-5 text-emerald-50/88">
              Public-safe preview does not expose Discord IDs, user IDs, source IDs, raw evidence, or exact award timestamps.
            </div>
          </section>
            ...
            <section className="rounded-lg border border-white/10 bg-white/[0.045] p-4">
              ...
              {cards.length ? (
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  {cards.map((card) => (
                    <CallingCard key={card.code} card={card} publicMode={publicMode} />
                  ))}
                </div>
              ) : (
            ...
            <section className="rounded-lg border border-white/10 bg-white/[0.045] p-4">
              ...
              <div className="mt-4 grid gap-3">
                {challenges.length ? challenges.map((challenge) => (
                  <ChallengeProgressCard key={challenge.id} challenge={challenge} publicMode={publicMode} />
          ...
          {!hiddenMode ? (
            <section className="rounded-lg border border-white/10 bg-white/[0.045] p-4">
              ...
              {timeline.length ? (
                <div className="mt-4 grid gap-2">
                  {timeline.map((item) => (
                    <TimelineRow key={item.id} item={item} publicMode={publicMode} />
                  ))}
                </div>
              ) : (
...
function CallingCard({ card, publicMode }: { card: ProfileCallingCard; publicMode: boolean }) {
  return (
    <article className="rounded-lg border border-violet-300/24 bg-[radial-gradient(circle_at_18%_0%,rgba(139,92,246,0.2),transparent_40%),rgba(0,0,0,0.32)] p-4">
      ...
      {!publicMode ? <p className="mt-3 text-xs font-bold text-zinc-500">Earned {formatDateTime(card.awarded_at)}</p> : null}
    </article>
  );
}

function ChallengeProgressCard({ challenge, publicMode }: { challenge: ProfileChallengeProgress; publicMode: boolean }) {
  ...
  return (
    <article className="rounded-lg border border-white/10 bg-black/28 p-4">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        {!publicMode && challenge.completed_at ? <Tag icon={<CalendarDays className="h-3 w-3" />} label={formatDateTime(challenge.completed_at)} /> : null}
      </div>
    </article>
  );
}

function TimelineRow({ item, publicMode }: { item: ProgressionTimelineItem; publicMode: boolean }) {
  return (
    <div className="flex items-start gap-3 rounded border border-white/10 bg-black/24 p-3">
      ...
      <span className="min-w-0">
        ...
        {!publicMode && item.occurred_at ? <span className="mt-1 block text-[10px] font-black uppercase text-zinc-600">{formatDateTime(item.occurred_at)}</span> : null}
      </span>
    </div>
  );
}
