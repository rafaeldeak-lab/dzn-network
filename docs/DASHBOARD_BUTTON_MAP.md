...
| Last ADM Import Report | Last Sync Details | Read-only `syncStatus.last_adm_import_report` | `GET /api/sync/status` | Owner/Admin | Shows parsed kills, written kills, duplicate skips, failed writes, cursor before/after, public cache status, and Discord queue count for the latest ADM import. | Owner can verify parser, DB writes, cursor, cache, and Discord queue behavior from one collapsed panel. | Shows nothing until an import report exists. | Working |
