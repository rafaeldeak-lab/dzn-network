...
| Check Nitrado Log Settings | Nitrado Log Settings checklist | `checkNitradoLogSettingsNow` | `GET /api/servers/[serverId]/nitrado-log-settings` | Owner/Admin | Uses the connected Nitrado service to verify Admin Log, Server Log, Reduce Log Output, and Log Playerlist where Nitrado exposes them. | Auto-saves verified settings and marks the checklist complete when required values are correct. | Falls back to manual confirmation with a clear warning. | Working |
