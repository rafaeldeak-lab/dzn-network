...
| Recover Stuck Sync Locks | Sync Lock Recovery panel | `recoverSyncLocks` | `POST /api/servers/[serverId]/sync/recover-locks` | Owner/Admin | Releases stale status locks older than 10 minutes and stale ADM locks older than 30 minutes for the selected server. Fresh active locks are left alone. | Before/after lock state is shown and Automation Health refreshes. | Shows action error and does not wipe stats/cache. | Working |
