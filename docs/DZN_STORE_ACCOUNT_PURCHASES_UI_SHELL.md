...
This slice consumes that contract without adding a new server route or schema. The UI treats the API as the only authority for Store purchase visibility:
...
- Private Supporter Card status cards that show only status, visibility state, selected theme key, supporter-since timestamp, issue/suspend/revoke timestamps, and the blocked reveal reason.
...
| Supporter Card private reveal | Absent | Absent | Absent | Absent |
...
It defines the future private reveal contract, serial/art redaction boundaries, account ownership proof, visibility controls, screenshot/export rules, audit requirements, rollback path, and security proof. Supporter Card serial/card-art reveal remains blocked until a later implementation slice.
