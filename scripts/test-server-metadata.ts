...
const metadataCallIndex = workflowSource.indexOf("/api/sync/metadata/run");
...
assert.equal(homepageSource.includes("value: dataPending ? \"Refreshing\" : formatNumber(playersOnline)"), true);
...
assert.equal(homeStatsSource.includes("SUM(COALESCE(server_public_cache.current_player_count, linked_servers.current_players, 0)) AS players_online"), true);
assert.equal(homeStatsSource.includes("SUM(COALESCE(server_public_cache.current_player_count, linked_servers.current_players, 0)) AS currentPlayersOnline"), true);
