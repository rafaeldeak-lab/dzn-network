...
The next safe implementation step should be the DZN Store public browse and Supporter Card preview contract. That slice should remain disabled by default and read-only unless separately approved.
...
| Future `/store`, `/account/purchases`, supporter-card, and `/wheel` surfaces | Store browse only if public store flag is enabled; purchase and wheel actions require login | Future guaranteed account-bound store purchases and earned-only wheel actions when flags are enabled | Same player/account store access; owner entitlement adds no store advantage | Same player/account store access; Pro adds no store advantage | Catalog draft model now adds only inactive `store_products` and `store_prices` plus local admin validation. No public `/store`, account purchases, checkout, webhook fulfilment, order/payment/entitlement tables, supporter-card issuance, earned-spin ledger, wheel runtime, live checkout, or issue #49 changes are implemented. Future store uses one-time checkout `mode=payment`, verified webhook fulfilment, immutable order/payment ledgers, account-bound cosmetic entitlements, refund/chargeback revocation, admin price controls, earned-only spin ledger, three-spins-per-24-hour cap, four-hour cooldown, no buyable spins, no owner setup entitlement, no ranking/discovery/review/event/Server Wars/CTF/XP/calling-card award or competitive effect, and no live checkout or issue #49 changes until separately approved |
