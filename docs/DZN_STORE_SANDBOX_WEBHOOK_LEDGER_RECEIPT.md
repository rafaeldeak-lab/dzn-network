...
Delivered follow-on reference: the DZN Store fulfilment ledger schema migration approval preflight in `docs/DZN_STORE_FULFILMENT_LEDGER_SCHEMA_PREFLIGHT.md` defines the future local/test fulfilment-ledger schema contract only. It adds no migration file, account entitlement table, Supporter Card table, Store fulfilment runtime, earned-spin ledger, wheel runtime, live checkout activation, production D1 write, Stripe mutation, Cloudflare config mutation, or issue #49 change.
