...
| Preview Parsed ADM | Manual ADM Import panel | `previewPastedAdmNow` | `POST /api/servers/[serverId]/adm/parse-preview` | Owner/Admin | Parses pasted ADM text without database writes. | Shows raw lines, raw kill lines, parsed kills, joins, disconnects, PlayerList snapshots, skipped dead-hit lines, and the first 10 kill previews. | Shows HTTP status, error code, details, and response body if preview fails. | Working |
| Import Pasted ADM Now | Manual ADM Import panel | `importPastedAdmNow` | `POST /api/servers/[serverId]/adm/manual-import` | Owner/Admin | Imports pasted ADM text or an uploaded `.ADM/.txt` file through the same parser/write path used by scheduled ADM processing. | Writes supported events, rebuilds stats, updates public cache, queues allowed Discord posts, saves Last ADM Import Report, shows a permanent Manual ADM Import Result panel, and force-refreshes dashboard sync/events/cache data. | Rejects empty filename/text or unauthorized users; repeated imports are deduped instead of double-counted. If dashboard refresh fails, the import result remains visible and Retry Dashboard Refresh is shown. | Working |
