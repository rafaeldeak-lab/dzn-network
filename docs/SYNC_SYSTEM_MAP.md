...
- `POST /api/servers/[serverId]/adm/import-job/start` powers job creation for **Import ADM Files Now**. The dashboard reads the uploaded ADM in the browser, splits it into 10-line chunks, and does not send full file text to one Worker invocation.
- `POST /api/servers/[serverId]/adm/import-job/chunk` processes exactly one small line chunk. Chunk requests only parse/write that line range and update job counters.
- `POST /api/servers/[serverId]/adm/import-job/finish` rebuilds stats, refreshes public cache, queues Discord posts, saves Last ADM Import Report, and completes the file after all chunks have been written.
- `POST /api/servers/[serverId]/adm/import-job/retry` resets a failed chunked job so the dashboard can retry the failed file. Event dedupe keeps retries from double-counting already-written rows.
- `GET /api/servers/[serverId]/adm/import-job/status?job_id=...` and `GET /api/servers/[serverId]/adm/import-job/latest?filename=...` let the dashboard reattach to an existing queued/processing job after a refresh or a stale failed client attempt.
- `POST /api/servers/[serverId]/adm/import-job/continue` processes the next server-side chunk for jobs that already have stored ADM text, such as scheduled Nitrado jobs. Manual browser-upload jobs can continue from the selected file object.
- The legacy full-file `POST /api/servers/[serverId]/adm/import-job` route now returns `chunked_import_required`.
- Hit feed import is optional and defaults off for manual file imports. This keeps Overview/Recent Synced Events focused on kills, deaths, joins, disconnects, and PlayerList activity, and avoids hundreds of hit rows in one import.
...
- If a 10-line chunk fails during dashboard import, the client retries that same range as 5-line chunks and then 1-line chunks. If one single line still fails, DZN records a parser/write warning, skips that line, and continues the file instead of failing the whole ADM import.
- Failed chunk responses include structured diagnostics: endpoint, chunk index, HTTP status, error code/message, response body, job id/status, first failed line number, and first failed line preview. Successful file results remain visible and failed files get a **Retry this file** action. Retrying a whole or partial batch is safe because event dedupe prevents already imported ADM rows from double-counting stats.
- Finish-phase issues after core chunk writes, such as public cache refresh, Discord queueing, sync-run history, or dashboard refresh warnings, are reported as `completed_with_warnings`. The file still counts as completed and the dashboard preserves accumulated parsed/written counters instead of showing zeroes.
- Active jobs are shown as processing, not failed. If Sync Health reports an active backend job, the Bulk ADM Import Summary reattaches to that job and overrides any stale failed client attempt for the same file. It counts processing files separately and displays current chunk, parsed kills, written kills, joins, disconnects, PlayerList snapshots, and the last job update while polling every 3 seconds.
...
- `POST /api/servers/[serverId]/adm/auto-sync-now` powers **Run Auto-Sync Now**, running one scoped Worker-style ADM auto-sync unit for the selected server and returning the same classified read/import status the scheduled worker records.
- Scheduled Nitrado processing now uses the same import job engine as manual file upload. When `/api/sync/adm/run` finds a newest readable ADM, it creates an `adm_import_jobs` row with `source = scheduled_nitrado`, stores the downloaded text as job input, and processes only a few 10-line chunks in that cron tick.
- `processPendingAdmImportJobs` runs on each ADM cron invocation and continues queued or retryable scheduled jobs before evaluating newly due servers. This prevents multi-thousand-line ADM files from being parsed, written, cached, and queued to Discord inside one Worker request.
- Scheduled chunk jobs rebuild stats, refresh public cache, queue allowed Discord posts, record the sync run, and save Last ADM Import Report only when the final chunk completes. Hit lines remain disabled by default for scheduled imports to avoid feed spam and D1 write pressure.
- A completed manual or scheduled import for the same server and ADM filename prevents noisy duplicate scheduled jobs. If a scheduled chunk fails, it is marked `failed_retryable`; written rows stay deduped and cron can resume safely.
