...
async function testPlayerHubRouteRuntimeContract() {
  ...
  db.competitiveEventServers.push({ event_id: "event-public", server_id: "server-public" });
  ...
  const payload = await response.json() as {
    ...
    suggested_events: Array<{ id: string; slug: string; status: string }>;
  ...
  assert.deepEqual(payload.matched_communities[0]?.matched_servers.map((server) => server.linked_server_id), ["server-public"], "Community server previews must include only public listed servers.");
  assert.deepEqual(payload.suggested_events.map((event) => event.id), ["event-public"], "Suggested events must include public active/upcoming events only.");
