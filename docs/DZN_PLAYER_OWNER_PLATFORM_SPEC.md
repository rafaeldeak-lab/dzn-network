...
26. Issue #49 live checkout activation: only after sandbox evidence, readiness review, production configuration review, migration safety, and explicit approval.
...
| `/store` preview, `POST /api/store/orders`, and future `/account/purchases`, supporter-card, and `/wheel` surfaces | Read-only Store preview only; order creation requires login and flags | Same read-only Store preview; authenticated local/test sandbox order creation may write pending player/account orders only when flags are explicitly enabled; future guaranteed account-bound store purchases and earned-only wheel actions require later flags | Same player/account Store preview and order boundary; owner entitlement adds no store advantage | Same player/account Store preview and order boundary; Pro adds no store advantage | Catalog draft model adds only inactive `store_products` and `store_prices` plus local admin validation. `/store` is now a disabled-by-default read-only preview using safe catalog metadata and Supporter Card preview copy only. The sandbox order ledger schema adds private local/sandbox `store_orders`, `store_order_items`, and `store_payment_events` tables with `livemode = 0`, one-item order snapshots, unique provider-event ids, sanitized summaries, and no-fulfilment blockers. DZN Store Sandbox Order Creation Route Approval Slice: `POST /api/store/orders` adds pending local/test order ledger writes only behind session auth, `DZN_STORE_SANDBOX_RUNTIME=local` or `test`, Store sandbox flags, live-checkout blockers, and catalog safety checks; checkout remains unavailable in the response. No Stripe Checkout Sessions, Store webhooks, `store_payment_events` writes, account entitlement writes, supporter-card issuance, earned-spin ledger, wheel runtime, production D1 apply, live checkout, or issue #49 changes are implemented. Future store uses one-time checkout `mode=payment`, verified webhook fulfilment, immutable order/payment ledgers, account-bound cosmetic entitlements, refund/chargeback revocation, admin price controls, earned-only spin ledger, three-spins-per-24-hour cap, four-hour cooldown, no buyable spins, no owner setup entitlement, no ranking/discovery/review/event/Server Wars/CTF/XP/calling-card award or competitive effect, and no live checkout or issue #49 changes until separately approved |
