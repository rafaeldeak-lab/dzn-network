...
assert.equal(getPlanByKey("network").server_status_interval_minutes, 5);
assert.equal(getPlanByKey("network").adm_discovery_interval_minutes, 10);
assert.equal(getPlanByKey("network").adm_pull_interval_minutes, 30);
assert.equal(getPlanByKey("premium").server_status_interval_minutes, 5);
assert.equal(getPlanByKey("premium").public_publish_interval_minutes, 1440);
assert.equal(getPlanByKey("premium").visibility_weight, 4);
assert.equal(getPlanByKey("partner").server_status_interval_minutes, 5);
assert.equal(getAdmDiscoveryIntervalMinutes("partner"), 10);
...
assert.equal(systemAuditSource.includes("STRIPE_PRICE_PREMIUM"), false);
...
assert.equal(syncMapDoc.includes("Legacy Premium, Network, and Partner rows are treated as effective Pro."), true);
