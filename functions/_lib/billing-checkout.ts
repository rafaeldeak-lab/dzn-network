...
export async function createOrResumeCheckout(env: Env, request: Request, input: {
  discordUserId: string; planKey: PurchasablePlanKey; priceId: string; returnTo: string;
}) {
  ...
  if (!attempt && input.planKey === "starter" && await findStarterTrialClaim(env, {
    ...
    attempt = await db.prepare("SELECT * FROM billing_checkout_attempts WHERE discord_user_id = ? AND state != 'closed' LIMIT 1")
      .bind(input.discordUserId).first<Attempt>();
    if (!attempt) throw new CheckoutRecoveryError("Starter trial has already been used or reserved for this account or Stripe customer. Use Manage billing or contact support.", "STARTER_TRIAL_UNAVAILABLE");
  }
  if (!attempt) {
    ...
    await db.prepare(`INSERT INTO billing_checkout_attempts
      ...
      .bind(id, input.discordUserId, input.planKey, customerId, mode, fingerprint, STRIPE_API_VERSION, JSON.stringify(params), now(), now()).run();
  ...
  if (attempt.plan_key !== input.planKey) throw planConflict();
  if (attempt.plan_key === "starter") await reserveAttemptTrial(env, attempt);
...
async function attachTrialSession(env: Env, attempt: Attempt, session: StripeCheckoutSession) {
  if (attempt.plan_key !== "starter") return;
...
async function closeExpiredAttempt(env: Env, attempt: Attempt) {
  ...
  ].slice(0, attempt.plan_key === "starter" ? 2 : 1));
}
