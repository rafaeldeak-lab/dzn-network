...
assert.match(route, /SELECT DISTINCT event_id, server_id[\s\S]+FROM competitive_event_servers/, "Suggested event relevance may read public event server links.");
assert.match(route, /server_id IN \(\$\{serverIds\.map\(\(\) => "\?"\)\.join\(", "\)\}\)/, "Suggested event relevance should only read the current player's followed or matched server IDs.");
