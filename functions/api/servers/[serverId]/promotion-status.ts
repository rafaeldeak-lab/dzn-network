...
import { getPromotionStatus, PromotionError } from "../../../_lib/server-promotions";
...
export const onRequestGet: PagesFunction = async ({ request, env, params }) => {
  ...
  try {
    const status = await getPromotionStatus(env, access.server.id);
    return json({
      ok: true,
      server: {
        id: access.server.id,
...
function unavailableResponse(error: unknown, serverId: string) {
  ...
  console.warn("DZN promotion status unavailable", {
    requestId,
    serverId,
    error: error instanceof Error ? error.message : "unknown",
  });
  return json(
    errorPayload("PROMOTION_STATUS_UNAVAILABLE", "Promotion status is temporarily unavailable. Please retry.", requestId),
    { status: 500 },
  );
}
