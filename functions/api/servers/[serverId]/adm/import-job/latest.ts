import { getLatestAdmImportJobProgressForFilename } from "../../../../../_lib/adm-sync";
...
export const onRequestGet: PagesFunction = async ({ request, env, params }) => {
  try {
    ...
    const job = await getLatestAdmImportJobProgressForFilename(env, { linkedServerId, filename });
    if (!job) return admImportJobError(404, "adm_import_job_not_found", "ADM import job not found.", { filename });
    return admImportJobJson({ ok: true, job });
  } catch (error) {
    return admImportJobError(500, "adm_import_job_latest_failed", "Unable to read latest ADM import job.", debugDetails(request, error));
  }
};
