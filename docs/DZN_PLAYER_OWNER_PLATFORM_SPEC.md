...
The future first implementation step should be a disabled-by-default DZN Store catalog and admin product/price draft model. That step may add reviewed local migrations and admin validation only. Checkout creation, webhook fulfilment, account entitlement writes, Supporter Card issuance, earned spins, wheel runtime, Stripe live mutation, Cloudflare secret mutation, production D1 mutation, and issue #49 remain out of scope until later approved payment slices.
