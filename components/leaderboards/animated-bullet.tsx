...
export function KillProjectileAccent({ tone }: { tone: "violet" | "cyan" | "orange" }) {
  const casingId = `dznKillCasing-${tone}`;
  const tipId = `dznKillTip-${tone}`;

  return (
    <span className={`dzn-kill-projectile dzn-kill-projectile--${tone}`} aria-hidden="true">
      <span className="dzn-kill-tracer dzn-kill-tracer--outer" />
      <span className="dzn-kill-tracer dzn-kill-tracer--inner" />
      {EMBERS.map((item) => (
