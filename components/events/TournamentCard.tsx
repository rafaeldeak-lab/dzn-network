...
export function TournamentCard({ event, compact = false }: { event: CompetitiveEvent; compact?: boolean }) {
  ...
  return (
    <article className="group overflow-hidden rounded-lg border border-white/10 bg-[#060a17]/92 shadow-[0_20px_70px_rgba(0,0,0,0.32)] transition hover:-translate-y-0.5 hover:border-violet-300/35 hover:shadow-[0_0_46px_rgba(124,58,237,0.18)]">
      <div className={compact ? "h-32 bg-cover bg-center" : "h-44 bg-cover bg-center"} style={eventImageStyle(event.banner_url)}>
      ...
      <div className="p-4">
        ...
        <h3 className="mt-1 text-lg font-black uppercase tracking-normal text-white">{event.name}</h3>
        ...
        <Link href={`/events/${event.slug}`} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-lg border border-violet-300/35 bg-violet-500/16 px-4 py-2.5 text-xs font-black uppercase text-violet-50 transition hover:bg-violet-500/26">
          View Event
