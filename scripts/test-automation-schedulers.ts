...
assert.equal(autoUpdateWorker.includes("async: true,\n      max_servers: 3"), false, "Metadata refresh must not be fire-and-forget; stale locks and public cache cleanup must finish.");
assert.equal(autoUpdateWorker.includes("source: \"cloudflare-live-metadata\""), true);
assert.equal(autoUpdateWorker.includes("deadline_ms: 8_000"), true);
assert.equal(autoUpdateWorker.includes("player_count_stale_ms: 60_000"), true);
assert.equal(autoUpdateWorker.includes("timeoutMs: 12_000"), true);
