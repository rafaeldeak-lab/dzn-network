...
After the follow-on catalog, public preview, checkout approval, sandbox order ledger schema, order route, Checkout Session route, and Store webhook receipt slices, that guard should remain active but explicitly allow only the catalog migration/helper files named above, `app/store/page.tsx`, `components/store/dzn-store-preview-page.tsx`, the local/sandbox-only `migrations/0072_dzn_store_order_ledger_schema.sql`, `POST /api/store/orders`, `POST /api/store/orders/:orderId/checkout`, and the receipt-only `POST /api/stripe/store-webhook`. It must continue to reject Store webhook fulfilment, account entitlements, supporter-card issuance, earned spins, wheel cooldowns, purchase Store UI, Stripe product/Price mutation, Cloudflare config changes, production D1 writes, live checkout activation, and issue #49 changes.
...
The DZN Store sandbox order creation route approval slice is now delivered in `docs/DZN_STORE_SANDBOX_ORDER_CREATION_ROUTE_APPROVAL.md`. It adds a disabled-by-default authenticated `POST /api/store/orders` route that writes pending local/test `store_orders` and `store_order_items` only after Store sandbox flags are explicitly enabled and catalog safety is verified. It creates no Stripe Checkout Sessions, processes no Store webhooks, writes no `store_payment_events`, grants no entitlements, issues no Supporter Cards, mints no earned spins, runs no wheel, mutates no Stripe objects, mutates no Cloudflare secrets/config, writes no production D1, enables no live checkout, and changes no issue #49.

The DZN Store sandbox Checkout Session approval slice is now delivered in `docs/DZN_STORE_SANDBOX_CHECKOUT_SESSION_APPROVAL.md`. It adds test-mode only Checkout Session creation through `POST /api/store/orders/:orderId/checkout` after an authenticated player already owns a pending local/test Store order. It uses a server-controlled test Price binding, order-derived idempotency, safe redirect URLs, and updates only `store_orders` to `checkout_created`. It adds no Store webhook fulfilment, entitlement write, Supporter Card issuance, earned-spin ledger, wheel runtime, live checkout activation, or issue #49 change. It also does not mutate Stripe Products or Prices, Cloudflare secrets/config, production D1, Nitrado, Discord, AI provider credentials, vector stores, analytics/tracking, or metered model calls.

The DZN Store sandbox webhook event ledger receipt slice is now delivered in `docs/DZN_STORE_SANDBOX_WEBHOOK_LEDGER_RECEIPT.md`. It adds disabled-by-default signed receipt handling through `POST /api/stripe/store-webhook`. It verifies Stripe signatures against the raw request body, accepts test-mode events only, and records sanitized `store_payment_events` receipt rows only. It adds no Store webhook fulfilment, account entitlement write, Supporter Card issuance, earned-spin ledger, wheel runtime, live checkout activation, Stripe Product/Price mutation, Cloudflare secret/config mutation, production D1 write, or issue #49 change.

The next Store payment step should be Store webhook fulfilment approval preflight only if deliberately approved: define the verified test-mode fulfilment contract, exact eligible events, order-status transitions, idempotent entitlement/supporter-card boundaries, refund/chargeback rollback rules, and proof matrix before any fulfilment route writes account entitlements, Supporter Cards, earned spins, wheel runtime, live checkout activation, Stripe Product/Price mutation, Cloudflare config mutation, production D1 writes, or issue #49 changes.

That approval preflight is now delivered in `docs/DZN_STORE_WEBHOOK_FULFILMENT_APPROVAL_PREFLIGHT.md`. The next Store payment step should be DZN Store fulfilment ledger schema migration implementation only if deliberately approved: add the local/test-only migration after rechecking migration numbering, create only the approved account-entitlement, Supporter Card, fulfilment-attempt, order-status-history, entitlement-status-history, and refund/dispute audit tables, keep Store fulfilment runtime disabled, and still avoid webhook fulfilment writes, Supporter Card issuance, earned spins, reward wheel runtime, live checkout, Stripe Product/Price mutation, Cloudflare config mutation, production D1 writes, and issue #49 changes.

The DZN Store fulfilment ledger schema migration implementation slice is now delivered in `docs/DZN_STORE_FULFILMENT_LEDGER_SCHEMA_MIGRATION.md` and `migrations/0073_dzn_store_fulfilment_ledger_schema.sql`. It creates only local/test-only private fulfilment ledger schema for account entitlements, Supporter Cards, fulfilment attempts, order-status history, entitlement-status history, and refund/dispute audit. The key runtime rule is explicit: runtime fulfilment remains disabled. It adds no Store webhook fulfilment write, Supporter Card issuance, earned spins, reward wheel runtime, live checkout activation, Stripe Product/Price mutation, Cloudflare config mutation, production D1 write, or issue #49 change.

The DZN Store fulfilment runtime implementation approval preflight is now delivered in `docs/DZN_STORE_FULFILMENT_RUNTIME_IMPLEMENTATION_PREFLIGHT.md`. It defines the future disabled-by-default local/test runtime sequence, exact write scope, idempotency behavior, order-status transitions, account-entitlement creation rules, optional Supporter Card issuance rules, refund/chargeback rollback rules, test matrix, rollback path, and security proof. It is documentation/test-guard work only and adds no Store fulfilment runtime, account entitlement write, Supporter Card issuance, earned-spin ledger, reward wheel runtime, live checkout activation, Stripe Product/Price mutation, Cloudflare config mutation, production D1 write, or issue #49 change.

The DZN Store fulfilment runtime implementation slice is now delivered in `docs/DZN_STORE_FULFILMENT_RUNTIME_IMPLEMENTATION.md`. It adds disabled-by-default local/test Store fulfilment runtime for verified `checkout.session.completed` receipts, idempotent `store_fulfilment_attempts`, status history, exactly one safe `account_entitlements` row per fulfilled source item, optional `supporter_cards` issuance only when independently flagged, and refund/dispute audit and rollback handling. The next Store payment step should be the Store fulfilment reconciliation/read-model preflight: define private Account Purchases and Entitlements read models, Supporter Card reveal/status UI contract, webhook replay/manual-review controls, and refund/dispute operator workflow before adding public reveal UI, account purchase routes, admin replay routes, notifications, production migration apply, live checkout activation, earned-spin ledger, reward wheel runtime, Stripe mutation, Cloudflare config mutation, production D1 write, or issue #49 change.

The DZN Store fulfilment reconciliation/read-model preflight is now delivered in `docs/DZN_STORE_FULFILMENT_RECONCILIATION_READ_MODEL_PREFLIGHT.md`. It defines future private Account Purchases and Entitlements read models, private Supporter Card reveal/status UI, admin-only webhook replay controls, admin-only manual-review controls, and admin-only refund/dispute operator workflow. It adds no public card reveal, account purchases route, entitlements route, admin replay route, manual-review route, refund/dispute operator route, notification, migration, production D1 apply, live checkout activation, earned-spin ledger, reward wheel runtime, Stripe mutation, Cloudflare config mutation, production D1 write, or issue #49 change.

The DZN Store Account Purchases read-model implementation is now delivered in `docs/DZN_STORE_ACCOUNT_PURCHASES_READ_MODEL_IMPLEMENTATION.md`. It adds disabled-by-default `GET /api/account/purchases` for private current-user Store purchase, entitlement, and Supporter Card status reads only. It is local/test sandbox only, private/no-store, sanitized, and read-only. It still adds no Account Purchases UI, no public or private Supporter Card reveal component, no Entitlements route, no operator route, no notification, no migration, no production D1 apply, no live checkout activation, no earned-spin ledger, no reward wheel runtime, no Stripe mutation, no Cloudflare config mutation, no production D1 write, and no issue #49 change.

The DZN Store Account Purchases UI shell is now delivered in `docs/DZN_STORE_ACCOUNT_PURCHASES_UI_SHELL.md`. It adds `/account/purchases` as an authenticated private read-only page that consumes only `GET /api/account/purchases`, uses no-store session fetches, handles disabled/read-model-unavailable states, and keeps Supporter Card reveal blocked. It adds no public or private Supporter Card reveal component, no Entitlements route, no webhook replay route, no manual-review route, no refund/dispute operator route, no notification, no migration, no production D1 apply, no live checkout activation, no earned-spin ledger, no reward wheel runtime, no Stripe mutation, no Cloudflare config mutation, no production D1 write, and no issue #49 change.

The DZN Store Supporter Card reveal approval preflight is now delivered in `docs/DZN_STORE_SUPPORTER_CARD_REVEAL_APPROVAL_PREFLIGHT.md`. It defines the future private reveal contract, serial/art redaction boundaries, account ownership proof, visibility controls, screenshot/export rules, audit requirements, rollback path, and security proof before any card reveal route is implemented. It adds no card reveal route, private reveal component, public reveal, card-art generation, sharing controls, screenshot/export controls, notifications, migrations, production D1 apply, live checkout activation, earned-spin ledger, reward wheel runtime, Stripe Product/Price/customer/refund/dispute/webhook endpoint mutation, Cloudflare config mutation, production D1 write, or issue #49 change.
