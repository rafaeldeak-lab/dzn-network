...
- Platform-owner connected-server support: the Command Centre inventory currently lists FED & FERAL and links to the normal owner dashboard, but that does not provide a dedicated support view. Add a platform-owner-only, server-scoped support dashboard for every connected server with the operational setup, lifecycle, sync, public-profile and account-linking context needed to assist its owner. Withhold Nitrado tokens and credentials, private Discord content, raw player coordinates/routes, payment secrets and other purpose-irrelevant sensitive data; audit access and support actions, and do not impersonate the server owner.
...
- Mobile/public UI: leaderboard availability, readable tables, record artwork, beta-banner clearance, correct Chernarus/Livonia/Sakhal map and aggregate-only privacy, understandable plan/map labels and named eligible Server Wars opponents. Scale the Server Wars live-motion hero responsively so the complete artwork remains visible at wide desktop and mobile proportions, with an intentional contained fallback/background treatment instead of cropping key imagery; preserve readable copy and reduced-motion behavior.
...
- Live Comms and support: the released Community surface remains a disabled read-only history preview. Implement authenticated global chat, moderated group rooms and the AI help assistant with server-enforced membership/visibility, rate limits, reporting and moderation, retention/privacy controls, owner/support escalation and audited sensitive access. Message sending, presence and AI runtime require their own tested backend contracts and production activation evidence; a visible chat box alone is not completion.
