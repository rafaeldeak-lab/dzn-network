...
assert.equal(authShellSource.includes("scroll-mb-6"), true, "Mission briefing cards should leave a small viewport-edge margin.");
...
assert.equal(authShellSource.includes("MOBILE_VIEWPORT_CLEARANCE_PX"), true, "Mission briefing tap scrolling should include viewport-edge clearance.");
...
assert.equal(authShellSource.includes("ensureBriefingDetailsInViewport"), true, "Mission briefing tap scrolling should keep expanded details in the viewport.");
assert.equal(authShellSource.includes("window.innerHeight - MOBILE_VIEWPORT_CLEARANCE_PX"), true, "In-flow ticker position must not define the bottom viewport boundary.");
assert.equal(authShellSource.includes("tickerTop"), false, "A ticker scrolled above the screen must not cause briefing scroll overshoot.");
