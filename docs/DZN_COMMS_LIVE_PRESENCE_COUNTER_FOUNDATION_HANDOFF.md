...
The current Comms follow-on is the DZN Comms reaction runtime implementation approval preflight in `docs/DZN_COMMS_REACTION_RUNTIME_IMPLEMENTATION_APPROVAL_PREFLIGHT.md`, building on the DZN Comms reaction interaction contract preflight in `docs/DZN_COMMS_REACTION_INTERACTION_CONTRACT_PREFLIGHT.md`: define exact routes, message-read prerequisites, storage/migration model, feature flags, idempotency/concurrency, rate limits, moderation scope, retention, rollback, and proof before any runtime reaction route, message table, reaction table, Durable Object, WebSocket, persistence, analytics/tracking, AI provider, vector store, metered model call, live checkout, production mutation, or issue #49 change.
