...
const jobLogRedirectStart = indexOfOrFail(auditScript, "async function requestGithubJobLogRedirect(jobId)");
const signedLogDownloadStart = indexOfOrFail(auditScript, "async function downloadSignedGithubJobLog(location)");
const getGithubJobLogStart = indexOfOrFail(auditScript, "async function getGithubJobLog(jobId)");
const workflowReferencesStart = indexOfOrFail(auditScript, "async function loadWorkflowReferences()");
const classifyDatabaseStart = indexOfOrFail(auditScript, "function classifyDatabase");
const writeJsonStart = indexOfOrFail(auditScript, "function writeJson");
const jobLogApiBlock = auditScript.slice(jobLogRedirectStart, signedLogDownloadStart);
const signedLogDownloadBlock = auditScript.slice(signedLogDownloadStart, getGithubJobLogStart);
const getGithubJobLogBlock = auditScript.slice(getGithubJobLogStart, workflowReferencesStart);
const workflowReferencesBlock = auditScript.slice(workflowReferencesStart, classifyDatabaseStart);
const artifactHandlingBlock = auditScript.slice(writeJsonStart, auditScript.indexOf("function appendStepSummary"));
...
assert.equal(auditScript.includes('githubHeaders("text/plain")'), false, "GitHub job-log API requests must not use text/plain.");
assert.equal(jobLogApiBlock.includes("headers: githubHeaders()"), true, "GitHub job-log API requests must use normal GitHub API headers.");
assert.equal(jobLogApiBlock.includes('redirect: "manual"'), true, "GitHub job-log API requests must not automatically follow signed-log redirects.");
assert.equal(jobLogApiBlock.includes("response.status === 200"), true, "GitHub job-log downloader must defensively support direct 200 responses.");
assert.equal(jobLogApiBlock.includes("githubLogRedirectStatuses"), true, "GitHub job-log downloader must handle redirect responses.");
assert.equal(auditScript.includes("const githubLogRedirectStatuses = new Set([301, 302, 303, 307, 308])"), true, "GitHub job-log downloader must recognize documented redirect statuses.");
assert.equal(jobLogApiBlock.includes('response.headers.get("location")'), true, "GitHub job-log downloader must read the redirect Location header.");
assert.equal(jobLogApiBlock.includes("validateSignedGithubLogLocation(location)"), true, "GitHub job-log redirects must be validated before fetching.");
assert.equal(auditScript.includes('parsed.protocol !== "https:"'), true, "Signed job-log URL validation must require https.");
assert.equal(auditScript.includes("isLoopbackOrPrivateHost(parsed.hostname)"), true, "Signed job-log URL validation must reject local/private targets.");
assert.equal(signedLogDownloadBlock.includes("fetch(location, {"), true, "Signed job-log URL must be fetched separately.");
assert.equal(signedLogDownloadBlock.includes('method: "GET"'), true, "Signed job-log URL fetch must remain read-only.");
assert.equal(signedLogDownloadBlock.includes('redirect: "follow"'), true, "Signed job-log URL fetch must follow storage redirects.");
assert.equal(signedLogDownloadBlock.includes("headers"), false, "Signed job-log URL fetch must not send headers.");
assert.equal(signedLogDownloadBlock.includes("githubHeaders"), false, "Signed job-log URL fetch must not use GitHub API headers.");
assert.equal(signedLogDownloadBlock.includes("Authorization"), false, "Signed job-log URL fetch must not send Authorization.");
assert.equal(auditScript.includes("const safeUnavailableLogStatuses = new Set([403, 404, 410, 415, 422])"), true, "Unavailable historical job logs must be represented by safe statuses.");
assert.equal(auditScript.includes("const retryableSignedLogStatuses = new Set([403, 404, 410])"), true, "Expired signed log downloads must be retryable only for expected statuses.");
assert.equal(auditScript.includes("const maxSignedLogAttempts = 2"), true, "Signed job-log downloads must be attempted at most twice.");
assert.equal(getGithubJobLogBlock.includes("signedAttempt <= maxSignedLogAttempts"), true, "Signed job-log retry loop must be bounded.");
assert.equal(getGithubJobLogBlock.includes("unexpected_log_error"), true, "Unexpected job-log errors must be sanitized into structured unavailability.");
assert.equal(jobLogApiBlock.includes("console.log"), false, "Signed job-log redirects must not be printed from the API helper.");
assert.equal(signedLogDownloadBlock.includes("console.log"), false, "Signed job-log URLs must not be printed from the download helper.");
assert.equal(getGithubJobLogBlock.includes("console.log"), false, "Signed job-log URLs must not be printed from the public job-log helper.");
assert.equal(/signed_url\s*:/.test(auditScript), false, "Signed job-log URLs must not be persisted as artifact fields.");
assert.equal(/location_header\s*:/.test(auditScript), false, "Redirect Location headers must not be persisted as artifact fields.");
assert.equal(auditScript.includes("raw_log_text"), false, "Raw job-log text must not be persisted in artifacts.");
assert.equal(workflowReferencesBlock.includes("job_log_summary"), true, "Workflow cross-reference must record sanitized job-log counts.");
assert.equal(workflowReferencesBlock.includes("unavailable_log_records"), true, "Workflow cross-reference must record unavailable job IDs safely.");
assert.equal(workflowReferencesBlock.includes("log_available"), true, "Workflow cross-reference must record log availability.");
assert.equal(workflowReferencesBlock.includes("safe_status"), true, "Workflow cross-reference must record safe log status.");
assert.equal(workflowReferencesBlock.includes("safe_reason"), true, "Workflow cross-reference must record safe log reason.");
assert.equal(workflowReferencesBlock.includes("if (logResult.available)"), true, "Unavailable logs must not crash workflow cross-reference.");
assert.equal(workflowReferencesBlock.includes("workflow_log_incomplete"), true, "Workflow cross-reference must flag incomplete log evidence.");
assert.equal(auditScript.includes("workflow_log_evidence_incomplete"), true, "Audit metadata must retain incomplete workflow-log evidence state.");
assert.equal(auditScript.includes("workflow cross-reference evidence is incomplete because one or more job logs were unavailable"), true, "Incomplete job-log evidence must classify affected resources as protected.");
assert.equal(auditScript.includes('classification: "UNKNOWN_PROTECTED"'), true, "Incomplete evidence must be able to produce UNKNOWN_PROTECTED.");
assert.equal(auditScript.includes('classification: "STALE_PREVIEW_CANDIDATE"'), true, "Stale candidates must remain explicit and guarded.");
assert.equal(auditScript.includes("workflowEvidenceIncomplete"), true, "Unavailable job logs must not support stale-candidate selection.");
assert.equal(artifactHandlingBlock.includes("initializeArtifacts"), true, "Audit must initialize sanitized artifacts early.");
assert.equal(artifactHandlingBlock.includes('status: "in_progress"'), true, "Audit metadata must support in-progress partial artifacts.");
assert.equal(artifactHandlingBlock.includes("writeFailureArtifacts"), true, "Audit must write a sanitized failure summary on fatal errors.");
assert.equal(artifactHandlingBlock.includes("failure_reason"), true, "Failure artifacts must record only sanitized failure reasons.");
assert.equal(auditScript.includes('["signed_url"'), true, "Artifact security scan must reject signed storage URLs.");
assert.equal(auditScript.includes('["location_header"'), true, "Artifact security scan must reject persisted redirect Location headers.");
...
assert.equal(auditScript.includes("const selected = staleCandidates.length === 1 ? staleCandidates[0] : null"), true, "Script 39 must select only one unambiguous candidate.");
...
function assertBillingPhase1PreviewWorkflowBoundary() {
...
assert.equal(billingPreviewScriptsBlock.includes("if (status === 500 && !options.allowExpectedHttp500) fail(\"BILLING_PREVIEW_HTTP_500\""), true, "Billing verifier must still fail unexpected HTTP 500 responses.");
...
const billingExpectedHttp500Guard = indexOfOrFail(billingRuntimeFailureBlock, "if (status === 500 && !options.allowExpectedHttp500)");
const billingRuntimeMarkerScan = indexOfOrFail(billingRuntimeFailureBlock, "for (const marker of forbiddenRuntimeText)");
const billingLeakMarkerScan = indexOfOrFail(billingRuntimeFailureBlock, "for (const marker of forbiddenLeakMarkers)");
const billingExactErrorCodeCheck = indexOfOrFail(billingPreviewScriptsBlock, "if (result.json?.error_code !== expectedCode)");
assert.equal(billingExpectedHttp500Guard < billingRuntimeMarkerScan, true, "Expected HTTP 500 responses must still pass through runtime marker checks.");
assert.equal(billingExpectedHttp500Guard < billingLeakMarkerScan, true, "Expected HTTP 500 responses must still pass through leak checks.");
assert.equal(billingExpectStatusStart < billingExactErrorCodeCheck, true, "Expected HTTP 500 responses must reach exact JSON error-code validation after runtime checks.");
assert.equal(billingPreviewScriptsBlock.includes("BILLING_PREVIEW_HTTP_503"), true, "Billing verifier must fail on HTTP 503.");
assert.equal(billingPreviewScriptsBlock.includes("if (status === 503) fail(\"BILLING_PREVIEW_HTTP_503\""), true, "Billing verifier must keep HTTP 503 fatal.");
...
assert.equal(billingPreviewScriptsBlock.includes("const shouldRetryTransient = transientStatuses.has(response.status) && attempt < attempts"), true, "Billing verifier must retry transient 404s before classifying missing Pages Functions.");
assert.equal(billingPreviewScriptsBlock.includes("if (!shouldRetryTransient) checkRuntimeFailure(path, response.status, body, options);"), true, "Billing verifier must classify API 404 after retry exhaustion.");
...
assertBillingPhase1PreviewWorkflowBoundary();
