...
export const onRequestGet: PagesFunction = async ({ request, env }) => {
  ...
  if (!user) return json({ ok: false, error: "unauthorized", items: [], selected: null }, { status: 401, headers: PULSE_NO_STORE_HEADERS });
  return json(await listEventPopups(env, user), { headers: PULSE_NO_STORE_HEADERS });
};
