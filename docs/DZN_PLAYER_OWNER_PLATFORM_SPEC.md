...
Next recommended product slice after public profile discovery/linking polish:

- Public profile owner preview/share polish: add a private current-user preview of how the public profile appears, copy/share controls from the account-owned surface, and richer hidden/not-yet-earned section states while keeping previews client-visible only and isolated from privacy writes unless explicitly saved, billing, rankings, discovery score, reviews, badges, seasons, events, Server Wars scoring, XP awards, calling-card awards, and competitive eligibility.
