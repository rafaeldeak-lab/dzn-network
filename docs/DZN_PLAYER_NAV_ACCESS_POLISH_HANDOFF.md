...
That slice is captured in `docs/DZN_COMMS_REACTION_INTERACTION_CONTRACT_PREFLIGHT.md`, the DZN Comms reaction runtime implementation approval preflight is captured in `docs/DZN_COMMS_REACTION_RUNTIME_IMPLEMENTATION_APPROVAL_PREFLIGHT.md`, and the DZN Comms message/read model approval preflight is captured in `docs/DZN_COMMS_MESSAGE_READ_MODEL_APPROVAL_PREFLIGHT.md`. Runtime reactions remain blocked until the message/read prerequisite and a later disabled local/test implementation slice are deliberately approved.
