...
After the presence counter foundation, message-adjacent reactions remain blocked until the dedicated DZN Comms reaction interaction contract preflight in `docs/DZN_COMMS_REACTION_INTERACTION_CONTRACT_PREFLIGHT.md`, the DZN Comms reaction runtime implementation approval preflight in `docs/DZN_COMMS_REACTION_RUNTIME_IMPLEMENTATION_APPROVAL_PREFLIGHT.md`, and the DZN Comms message/read model approval preflight in `docs/DZN_COMMS_MESSAGE_READ_MODEL_APPROVAL_PREFLIGHT.md` are complete. The runtime approval preflight defines exact reaction routes, message-read prerequisites, migration/storage shape, feature flags, retention, moderation scope, per-user idempotency, concurrency behavior, rollback, and proof before any runtime reaction API, message table, reaction table, Durable Object, WebSocket, persistence, analytics/tracking, AI provider, vector store, metered model call, production mutation, live checkout, or issue #49 change.
