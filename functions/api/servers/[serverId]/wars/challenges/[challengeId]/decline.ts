import { getSessionUser } from "../../../../../../_lib/db";
import { json, methodNotAllowed } from "../../../../../../_lib/http";
import { declineServerWarChallenge } from "../../../../../../_lib/server-wars";
import type { PagesFunction } from "../../../../../../_lib/types";

export const onRequestPost: PagesFunction = async ({ request, env, params }) => {
  const serverId = sanitizeParam(params.serverId);
  const challengeId = sanitizeParam(params.challengeId);
  if (!serverId || !challengeId) return json({ ok: false, error: "invalid_request" }, { status: 400 });
  const user = await getSessionUser(env, request);
  if (!user) return json({ ok: false, error: "unauthenticated" }, { status: 401 });
  const result = await declineServerWarChallenge(env, user, serverId, challengeId);
  if (!result.ok) return json({ ok: false, error: result.error }, { status: result.status });
  return json(result);
};

export const onRequestGet: PagesFunction = () => methodNotAllowed();
export const onRequestPut: PagesFunction = () => methodNotAllowed();
export const onRequestDelete: PagesFunction = () => methodNotAllowed();

function sanitizeParam(value: unknown) {
  const raw = Array.isArray(value) ? value[0] : value;
  return typeof raw === "string" ? raw.trim().slice(0, 96) : "";
}
