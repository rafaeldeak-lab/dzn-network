...
import { leaveOwnerEvent } from "../../../../../_lib/event-hub";
...
export const onRequestPost: PagesFunction = async ({ request, env, params }) => {
  ...
  if (!user) return json({ ok: false, error: "NOT_AUTHENTICATED", message: "Log in to leave events." }, { status: 401 });
  ...
  const result = await leaveOwnerEvent(env, user, serverId, eventId);
