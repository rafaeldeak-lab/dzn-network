...
- `docs/DZN_COMMS_INTERACTION_CONTRACT_PREFLIGHT.md`
  ...
  - Points the dedicated message/read prerequisite to `docs/DZN_COMMS_MESSAGE_READ_MODEL_APPROVAL_PREFLIGHT.md` before any real message-history route or reaction runtime can be implemented.
  - Points runtime emoji reactions to the dedicated `docs/DZN_COMMS_REACTION_INTERACTION_CONTRACT_PREFLIGHT.md` contract and the DZN Comms reaction runtime implementation approval preflight in `docs/DZN_COMMS_REACTION_RUNTIME_IMPLEMENTATION_APPROVAL_PREFLIGHT.md`. No runtime reaction behavior, message table, reaction table, persistence, analytics/tracking, or issue #49 change is introduced here.
...
The dedicated reaction contract preflight, reaction runtime approval preflight, and DZN Comms message/read model approval preflight are now captured separately. Runtime reactions remain blocked until the message/read prerequisite exists and a later disabled local/test implementation slice uses the exact routes, storage model, feature flags, retention, moderation scope, idempotency strategy, rollback, and proof matrix from `docs/DZN_COMMS_REACTION_RUNTIME_IMPLEMENTATION_APPROVAL_PREFLIGHT.md`.
