import { retryAdmImportLineJobForServer } from "../../../../../_lib/adm-sync";
import { getSessionUser } from "../../../../../_lib/db";
import { json, methodNotAllowed, readJson } from "../../../../../_lib/http";
import { requireServerOwnerOrDznAdmin } from "../../../../../_lib/public-cache";
import type { PagesFunction } from "../../../../../_lib/types";

type RetryAdmImportJobBody = {
  jobId?: string;
};
...
export const onRequestPost: PagesFunction = async ({ request, env, params }) => {
  try {
    ...
    const body = await readJson<RetryAdmImportJobBody>(request);
    const jobId = sanitizeJobId(body.jobId);
    if (!jobId) return admImportJobError(400, "missing_job_id", "ADM import job id is required.");
    const result = await retryAdmImportLineJobForServer(env, { linkedServerId, jobId });
    return admImportJobJson(result);
  } catch (error) {
    return admImportJobError(500, "adm_import_job_retry_failed", "Unable to reset ADM import job for retry.", debugDetails(request, error));
  }
};
