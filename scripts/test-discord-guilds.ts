...
const onboardingProofSource = readFileSync("functions/_lib/onboarding-service-proof.ts", "utf8");
assert.equal(onboardingTestSource.includes("getOnboardingServiceProof"), true);
assert.equal(onboardingProofSource.includes("Token encryption key is missing in production. Add TOKEN_ENCRYPTION_KEY in Cloudflare Pages and redeploy."), true);
assert.equal(onboardingProofSource.includes("Your saved Nitrado token cannot be decrypted. Re-save your Nitrado long-life token."), true);
