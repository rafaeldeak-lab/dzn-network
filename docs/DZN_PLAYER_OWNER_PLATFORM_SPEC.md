...
The player-facing flow allows a logged-in player to request a link from `/player/profile` without needing an internal server slug or hidden ADM player ID. The UI presents a public server picker/search and asks for the visible DayZ gamertag used on that server. Server-side resolution is restricted to that exact `linked_server_id`: an exact hidden `player_id` match is accepted for backwards compatibility, otherwise a case-insensitive exact gamertag may identify a review candidate only when one imported `player_profiles` row matches. Missing and duplicate-name matches fail closed. A server owner for that server or a DZN admin must then approve or reject the pending claim through private owner/admin routes. Approval revalidates the stored `player_profile_id + linked_server_id + player_id` and creates exactly one active verified link; a gamertag never creates or approves a link by itself.

Player-facing copy should avoid technical terms such as "slug", "linked_server_id", and "ADM player_id" unless the user is in an owner/admin technical surface. The safe mental model for normal players is: choose the server, enter the same gamertag shown there, then wait for owner/admin approval. A unique server-scoped name is lookup evidence only; ownership and the verified identity are never inferred from the name.
...
- Never approve or attach a game identity from `player_name`, public profile handle, display name, review name, Discord username, or leaderboard text. A unique exact gamertag may only locate a pending review candidate inside one selected server.
...
Authenticated Global Chat sending is defined in [DZN Comms Live Moderation Contract](DZN_COMMS_MESSAGE_SENDING_CONTRACT.md), with a machine-checkable JSON companion. The default-off implementation adds same-origin Discord-session writes, deterministic filtering/timeouts, durable idempotency and quota ledgers, reports and platform-owner audit actions. Production migrations `0065_dzn_comms_read_history.sql`, `0071_dzn_comms_live_moderation.sql`, and `0072_dzn_comms_private_rate_ledgers.sql`, production flags and rendered live verification remain separate release operations. Private sending still needs its own membership/revocation proof.
...
- Shared header compact sizing polish: the command-deck header is scaled down across shared pages while preserving the animated logo, hover/focus highlighting, Player Hub access, owner actions, and bright red DZN Pulse unread badges; visual only with no payment, Store, chat runtime, production, or competitive-system changes.
...
- Keep the remaining DZN Comms/support work separate: production activation and retention cleanup for Global Chat, the site-wide support launcher, private groups, reactions, live presence, a dedicated moderation screen and the public-DZN-info-only AI support bot still need their own release slices. AI credentials, vector stores, analytics/tracking, metered calls, live checkout and Store/payment changes remain outside Comms.
