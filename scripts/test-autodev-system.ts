...
assert.equal(config.automationPolicy.low.allowedToInvestigate, true);
assert.equal(config.automationPolicy.low.allowedToImplementInBranch, true);
assert.equal(config.automationPolicy.low.allowedToOpenPR, true);
assert.equal(config.automationPolicy.low.allowedToAutoMerge, false);
assert.equal(config.automationPolicy.medium.allowedToAutoMerge, false);
assert.equal(config.automationPolicy.high.allowedToAutoMerge, false);
assert.equal(config.automationPolicy.high.allowedToDeployProduction, false);
assert.equal(config.automationPolicy.blocked.allowedToImplementInBranch, false);
...
assert.equal(classifyPath("app/events/page.tsx", "export default function Page() { return <main />; }").risk, "low");
assert.equal(classifyPath("functions/api/events/matchmaking.ts", "createCategorySafeMatchmaking").risk, "medium");
...
assert.equal(classifyPath("functions/api/stripe/webhook.ts").risk, "high");
...
assert.equal(classifyPath("migrations/9999_truncate.sql", "TRUNCATE TABLE player_profiles;").risk, "blocked");
assert.equal(classifyPath("components/onboarding/dashboard.tsx", "export function Dashboard() { return <p className=\"truncate\">Billing status</p>; }").risk, "high");
assert.notEqual(classifyPath("scripts/autodev/risk-classifier.ts", read("scripts/autodev/risk-classifier.ts")).risk, "blocked");
...
assert.equal(classifyPath("migrations/9999_billing.sql", "ALTER TABLE subscriptions ADD COLUMN test TEXT;").risk, "high");
assert.equal(classifyPath("migrations/9999_events.sql", "ALTER TABLE competitive_events ADD COLUMN test TEXT;").risk, "medium");
...
for (const workflowName of ["DZN ADM AutoDev Audit", "DZN ADM Post Deploy Verify", "DZN ADM Cycle Watch", "DZN Codex Safe Fix", "DZN ADM Worker Manual Trigger", "DZN Nitrado ADM Diagnostics"]) {
...
const canonicalProductionUrlExpression = "DZN_APP_URL: ${{ vars.DZN_APP_URL || 'https://dayz-network.com' }}";
const cloudflareTokenExpression = "CLOUDFLARE_API_TOKEN: ${{ secrets.CLOUDFLARE_API_TOKEN }}";
const cloudflareAccountExpression = "CLOUDFLARE_ACCOUNT_ID: ${{ secrets.CLOUDFLARE_ACCOUNT_ID || vars.CLOUDFLARE_ACCOUNT_ID }}";
assert.equal(read(".autodev/config.json").includes('"productionUrl": "https://dayz-network.com"'), true);
assert.equal(postDeployWorkflow.includes(canonicalProductionUrlExpression), true);
assert.equal(cycleWatchWorkflow.includes(canonicalProductionUrlExpression), true);
assert.equal(postDeployWorkflow.includes(cloudflareTokenExpression), true);
assert.equal(postDeployWorkflow.includes(cloudflareAccountExpression), true);
assert.equal(cycleWatchWorkflow.includes(cloudflareTokenExpression), true);
assert.equal(cycleWatchWorkflow.includes(cloudflareAccountExpression), true);
assert.equal((postDeployWorkflow.match(/CLOUDFLARE_API_TOKEN:/g) ?? []).length, 1);
assert.equal((cycleWatchWorkflow.match(/CLOUDFLARE_API_TOKEN:/g) ?? []).length, 1);
...
assert.equal(admLiveVerify.includes('process.env.DZN_APP_URL || "https://dayz-network.com"'), true);
assert.equal(admLiveVerify.includes("Refusing non-read-only production D1 statement."), true);
assert.equal(admLiveVerify.includes("ALTER|ATTACH|CREATE|DELETE|DETACH|DROP|INSERT|PRAGMA|REPLACE|UPDATE|UPSERT|VACUUM"), true);
...
assert.equal(admLiveVerify.includes("healthFileHasDurableAttemptEvidence"), true);
assert.equal(admLiveVerify.includes("durable file-state evidence"), true);
assert.equal(admLiveVerify.includes("Number(file.retryCount ?? 0) > 0"), true);
