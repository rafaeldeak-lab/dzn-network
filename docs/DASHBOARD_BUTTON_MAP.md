...
| Import Pasted ADM Now | Manual ADM Import panel | `importPastedAdmNow` | `POST /api/servers/[serverId]/adm/manual-import` | Owner/Admin | Imports pasted ADM text or an uploaded `.ADM/.txt` file through the same parser/write path used by scheduled ADM processing. | Writes supported events, rebuilds stats, updates public cache, queues allowed Discord posts, saves Last ADM Import Report, and shows parsed/written counts. | Rejects empty filename/text or unauthorized users; repeated imports are deduped instead of double-counted. | Working |
