...
The implementation preflight is `docs/DZN_SAFE_MONETISATION_SUPPORTER_IMPLEMENTATION_PREFLIGHT.md`. It defines the real production sequence before runtime work starts:

- Catalog and admin draft schema.
- Store browse and Supporter Card preview UI.
- Authenticated local order creation.
- One-time Stripe Checkout using `mode=payment` only after checkout flags are enabled.
- Raw-body signed webhook verification.
- Idempotent payment-event recording and fulfilment.
- Refund, reversal, and chargeback revocation.
- Tax/receipt records.
- Admin pricing and product availability controls.
- Earned-only spin ledgers and reward wheel cooldown enforcement.
- Rollback and security proof.

The preflight is documentation/test guard work only. It does not add checkout routes, webhook fulfilment, product/order/payment/spin migrations, feature flags to runtime environment files, account entitlement writes, Supporter Card issuance, earned-spin rows, reward wheel runtime, Stripe product/Price changes, Cloudflare secret changes, production D1 writes, Nitrado changes, Discord changes, AI provider credentials, vector stores, analytics/tracking, metered model calls, live checkout activation, or issue #49 changes.

The first implementation step after this preflight is the disabled-by-default DZN Store catalog and admin product/price draft model. That step may add reviewed local migrations and admin validation only. Checkout creation, webhook fulfilment, account entitlement writes, Supporter Card issuance, earned spins, wheel runtime, Stripe live mutation, Cloudflare secret mutation, production D1 mutation, and issue #49 remain out of scope until later approved payment slices.

## DZN Store Catalog And Admin Product/Price Draft Model Slice

The DZN Store catalog and admin product/price draft model slice adds `migrations/0071_dzn_store_catalog_admin_draft.sql` and `functions/_lib/dzn-store-catalog.ts` as the first implementation step after the Safe Monetisation preflight.

Allowed behavior:

- Create the inactive `store_products` catalog table.
- Create the inactive `store_prices` catalog table.
- Keep catalog rows account-bound, guaranteed, and explicitly no-competitive-advantage.
- Keep all product and price rows inactive by default.
- Add local admin draft validation that requires Store/admin flags plus a previously admin-authorized caller.
- Preserve the `DZN FOUNDING SUPPORTER PACK` as a supporter product draft only, without issuing cards.
...
- No order, payment event, account entitlement, supporter card, earned-spin, spin-ledger, or wheel-cooldown tables.
- No Supporter Card issuance.
- No earned-spin or wheel runtime.
- No Stripe product/Price mutation or binding.
- No Cloudflare secret/config mutation.
- No production D1 write.
- No live checkout activation.
- No issue #49 change.

The validation rejects active product/price drafts, Stripe Price IDs, pay-what-you-want pricing, incompatible product fulfilment, and product copy or flags that would grant paid spins, XP, ranking, discovery, review, event, Server Wars, CTF, owner setup, Nitrado, or competitive eligibility benefits.

## DZN Store Public Browse And Supporter Card Preview Contract Slice

The DZN Store public browse and Supporter Card preview contract slice adds `/store` as a disabled-by-default, read-only preview route. It renders safe static catalog preview metadata from `functions/_lib/dzn-store-catalog.ts` through `components/store/dzn-store-preview-page.tsx`.

Allowed behavior:

- Show preview-only product cards for the Founding Supporter Pack, profile themes, cosmetic calling-card packs, chat/profile cosmetics, group banner and insignia cosmetics, and event presentation themes.
- Show guaranteed-purchase, account-bound, no-competitive-advantage, and no-buyable-spin labels.
- Show a sample-only `DZN FOUNDING SUPPORTER PACK` card preview with sample serial `DZN-SUP-002481`, theme options, display name, Supporter Since copy, and generated-insignia copy.
- Show that checkout is disabled and that issue #49 remains untouched.
- Use DZN-branded cinematic background layers with reduced-motion fallback.
...
- No account purchases page, Store API route, payment success fulfilment, AI provider, vector store, analytics, tracking, Nitrado, or Discord mutation.
...
It adds the private local/sandbox schema for `store_orders`, `store_order_items`, and `store_payment_events`. These are sandbox/local scoped with `livemode = 0` and are not production-applied by this slice.

Allowed behavior:

- Create a local/sandbox `store_orders` table for private order headers, session-derived purchasing user references, hashed Discord/customer references, immutable product/price/tax/Store-flag snapshots, one product per order, and provider ids needed for later sandbox reconciliation.
- Create a local/sandbox `store_order_items` table for exactly one item per order, quantity `1`, product/price references, immutable item snapshots, account-bound guaranteed-purchase constraints, and no-competitive-advantage paid-outcome checks.
- Create a local/sandbox `store_payment_events` table for unique Stripe event ids, provider reference ids, processing status, raw event SHA-256 hash, sanitized summary JSON, and failure details.
- Fix `store_payment_events` fulfilment, entitlement-write, and Supporter Card write attempt columns to `0` so this schema cannot be used as a fulfilment path.
- Keep Store checkout flags disabled by default and avoid adding Cloudflare env/config entries in this slice.

Blocked behavior:

- No `POST /api/store/orders`.
- No `POST /api/stripe/store-webhook`.
- No checkout route or Store API.
- No Stripe Checkout Session creation.
- No Store webhook handler or webhook fulfilment.
- No account entitlement table or write.
- No Supporter Card table or issuance.
- No earned-spin ledger, spin ledger, wheel cooldown, or reward wheel runtime.
- No Stripe object mutation, Cloudflare secret/config mutation, production D1 write, live checkout activation, or issue #49 change.

The schema is payment-ledger preparation only. It cannot affect owner entitlement, server ownership, `/setup`, Nitrado access, rankings, discovery, reviews, badges, seasons, events, Server Wars, CTF scoring, XP awards, earned calling-card awards, public profile visibility, retained exports, moderation decisions, or competitive eligibility.

## DZN Store Sandbox Order Creation Route Approval Slice

The DZN Store sandbox order creation route approval slice is delivered in `docs/DZN_STORE_SANDBOX_ORDER_CREATION_ROUTE_APPROVAL.md`. It adds authenticated `POST /api/store/orders` pending local/test order ledger writes only, and checkout remains unavailable in the response.

## DZN Store Sandbox Checkout Session Approval Slice

The DZN Store sandbox Checkout Session approval slice is delivered in `docs/DZN_STORE_SANDBOX_CHECKOUT_SESSION_APPROVAL.md`.

It adds `POST /api/store/orders/:orderId/checkout` as a private authenticated player route. The route can create a test-mode Stripe Checkout Session only after an owned draft local/test Store order exists, Store sandbox flags are explicitly enabled, `DZN_STORE_SANDBOX_CHECKOUT_SESSION_ENABLED=true`, `STRIPE_SECRET_KEY` is test-mode only, live-checkout flags are false, and the product/item/price/order row preserves the Fair Progression Boundary.

Allowed behavior:

- Read the existing pending `store_orders`, `store_order_items`, and `store_prices` rows scoped to the current session user.
- Use one server-controlled `store_prices.stripe_price_id`.
- Create one test-mode Stripe Checkout Session using `mode=payment`.
- Use an order-derived non-sensitive Stripe idempotency key.
- Update only `store_orders` to `checkout_created`.
- Return the Stripe redirect URL while keeping `session_id: null` and hiding customer, PaymentIntent, raw Discord, and raw DZN user ids.

Blocked behavior:

- No Store webhook processing or webhook fulfilment.
- No `store_payment_events` write.
- No account entitlement write.
- No Supporter Card issuance.
- No earned-spin ledger or reward wheel runtime.
- No owner subscription, owner entitlement, `/setup`, Nitrado, server-management, or server-ownership change.
- No Stripe Product/Price/Customer/webhook/refund/dispute mutation.
- No Cloudflare secret/config mutation, production D1 write, live checkout activation, or issue #49 change.

Success redirects still do not fulfil purchases. Fulfilment remains blocked until a later verified-webhook slice is deliberately approved.

## DZN Store Sandbox Webhook Event Ledger Receipt Slice

The DZN Store sandbox webhook event ledger receipt slice is delivered in `docs/DZN_STORE_SANDBOX_WEBHOOK_LEDGER_RECEIPT.md`.

It adds `POST /api/stripe/store-webhook` as a disabled-by-default Stripe-signed receipt route. The route verifies the `Stripe-Signature` header against the unmodified raw request body, accepts only test-mode events, and writes sanitized `store_payment_events` rows only. This is a verified Stripe signature boundary.

Allowed behavior:

- Read Store sandbox flags and `STRIPE_WEBHOOK_SECRET` from local/test runtime only.
- Verify Stripe signatures before parsing.
- Reject missing, malformed, mismatched, or live-mode events before ledger writes.
- Record unique Stripe event ids with `ON CONFLICT(stripe_event_id) DO NOTHING`.
- Store event class, optional safe provider references, raw event SHA-256 hash, and sanitized summary JSON.
- Keep fulfilment, entitlement-write, and Supporter Card write attempt columns fixed to `0`.

Blocked behavior:

- No Store webhook fulfilment.
- No order fulfilment or order status update.
- No account entitlement write.
- No Supporter Card issuance.
- No earned-spin ledger or reward wheel runtime.
- No owner subscription, owner entitlement, `/setup`, Nitrado, server-management, or server-ownership change.
- No Stripe Product/Price/Customer/Checkout Session/refund/dispute/webhook endpoint mutation.
- No Cloudflare secret/config mutation, production D1 write, live checkout activation, or issue #49 change.

Receipt rows are private payment-ledger artifacts. They cannot affect billing, owner entitlement, server ownership, rankings, discovery, reviews, badges, seasons, events, Server Wars, CTF scoring, XP awards, earned calling-card awards, public profile visibility, retained exports, moderation decisions, or competitive eligibility.

No fulfilment, entitlements, Supporter Cards, earned spins, reward wheel runtime, production D1 write, live checkout, or issue #49 change is part of this slice.
...
## DZN Store Webhook Fulfilment Approval Preflight Slice

The DZN Store webhook fulfilment approval preflight is a documentation/test-guard slice only. It defines the future verified test-mode Store fulfilment contract before any account entitlement, Supporter Card, earned-spin, or wheel runtime exists.

The current Store webhook route remains receipt-only:

- `POST /api/stripe/store-webhook`
- `functions/api/stripe/store-webhook.ts`
- `functions/_lib/dzn-store-webhook.ts`

The current `store_payment_events` fulfilment blockers remain fixed to `0`, and the Store ledger still has no `account_entitlements`, `supporter_cards`, `earned_spins`, `spin_ledger`, or `wheel_cooldowns` tables.

The preflight defines:

- `checkout.session.completed` as the first eligible future test-mode fulfilment event when the signed Checkout Session is `mode=payment`, complete, paid, tied to the local order, and matches the order/item/price snapshots.
...
- Success-page redirects as never enough to grant entitlements.
- Order transitions through `checkout_created`, `payment_pending`, `paid`, `payment_failed`, `checkout_expired`, `disputed`, `refunded`, `revoked`, `manual_review`, and `blocked_by_flag`.
- Exactly one account entitlement per fulfilled source order.
- Exactly one Founding Supporter Card per qualifying account.
- Full refund, reversal, chargeback, and lost-dispute rollback for only the affected Store entitlement/card.
- Partial refund handling as `manual_review` unless a later policy deliberately supports it.
- A future schema approval requirement before any entitlement/supporter-card table, fulfilment-attempt write, refund/dispute revocation audit, or runtime side effect exists.
...
The DZN Store fulfilment ledger schema migration implementation slice adds `migrations/0073_dzn_store_fulfilment_ledger_schema.sql` as local/test-only fulfilment ledger schema.
...
The schema is private Store infrastructure for a future verified webhook fulfilment runtime. It enforces local/sandbox ledger scope, `livemode = 0`, account-bound cosmetic/supporter entitlement constraints, one Founding Supporter Card per qualifying account, unique `DZN-SUP-######` serials, fulfilment-attempt idempotency, non-destructive status history, refund/dispute audit, and fixed-zero no-competitive-advantage entitlement fields.

No runtime fulfilment, Supporter Card issuance, earned spins, reward wheel runtime, live checkout, production D1 apply, or issue #49 change is implemented.

Fairness remains unchanged: this schema cannot affect owner billing plan status, `/setup`, Nitrado linking, server ownership, rankings, discovery score, reviews, badges, seasons, events, CTF scoring, Server Wars scoring, XP awards, calling-card awards, public profile visibility, retained exports, moderation decisions, or competitive eligibility.

Live checkout remains disabled, and Issue #49 remains reserved for final live payment activation.

## DZN Store Fulfilment Runtime Implementation Approval Preflight Slice
...
It defines the future disabled-by-default local/test runtime contract only. That contract covers the verified Store webhook sequence, feature flag requirements, exact write scope, idempotent `store_fulfilment_attempts` behavior, order-status transitions, account-entitlement creation rules, optional Supporter Card issuance rules, refund/reversal/chargeback/dispute rollback, privacy boundaries, test matrix, rollback path, and security proof.

The current Store webhook remains receipt-only. No Store fulfilment runtime, account entitlement write, Supporter Card issuance, earned-spin ledger, reward wheel runtime, live checkout activation, Stripe Product/Price mutation, Cloudflare config mutation, production D1 write, or issue #49 change is implemented.
...
The DZN Store Account Purchases read-model implementation is delivered in `docs/DZN_STORE_ACCOUNT_PURCHASES_READ_MODEL_IMPLEMENTATION.md`.

It adds `functions/api/account/purchases.ts` and `functions/_lib/dzn-store-account-purchases.ts` for `GET /api/account/purchases`.

The route is disabled by default behind `DZN_STORE_ACCOUNT_PURCHASES_READ_MODEL_ENABLED=false`, requires `DZN_STORE_ENABLED=true`, requires `DZN_STORE_SANDBOX_RUNTIME=local` or `test`, blocks live checkout flags, blocks earned-spin/reward-wheel runtime flags, and requires an authenticated DZN session before any Store purchase data is returned.

The route reads only the current user's sanitized Store ledgers for `livemode = 0` and the active local/test sandbox ledger scope. It may show customer-facing order number, order/product/price snapshot status, entitlement status, private Supporter Card status, display-safe status history, sanitized payment receipt state, fulfilment attempt state, and refund/dispute decision summaries. It must not expose raw Stripe ids, payment method data, customer email, billing address, raw webhook bodies, raw provider payloads, raw Discord ids, raw internal DZN user ids, raw internal Store order ids, raw order item ids, raw entitlement ids, raw Supporter Card ids, Supporter Card serial numbers, operator notes, other sandbox-scope rows, or any other user's records.

This implementation adds no Account Purchases UI page, no public Supporter Card reveal, no private Supporter Card reveal component, no Entitlements route, no webhook replay route, no manual-review route, no refund/dispute operator route, no notification, no migration, no production D1 apply, no live checkout activation, no earned-spin ledger, no reward wheel runtime, no Stripe mutation, no Cloudflare config mutation, no production D1 write, and no issue #49 change.
...
The DZN Store Supporter Card private reveal implementation is delivered in `docs/DZN_STORE_SUPPORTER_CARD_REVEAL_IMPLEMENTATION.md`.

It adds `functions/api/account/supporter-cards/[cardRef]/reveal.ts` and `functions/_lib/dzn-store-supporter-card-reveal.ts` as a disabled-by-default local/test private route for the current authenticated account. The route can reveal the Supporter Card serial/status only after joined Store order, Store order item, account entitlement, and Supporter Card ownership proof for the current user.

The Account Purchases read model can now advertise private reveal availability behind `DZN_SUPPORTER_CARD_PRIVATE_REVEAL_ENABLED=true`, but the list/status read model still does not return Supporter Card serial numbers or generated card art. `/account/purchases` includes a private reveal panel that fetches only this route with no-store session credentials and stores reveal state only in component memory for the current page session.
...
| `/api/dzn-comms/channels/:channelId/messages` and future `/community` message-history UI | Static fallback only; no message-history API read | Visible public-channel messages only when local/test read flags and the future UI flag are enabled; no payment required | Same as free player; no paid read advantage | Same as Starter; Pro adds no read advantage | Message-history reads are local/test, disabled by default, read-only, no-store, and presentation-only. Private groups require active trusted `dzn_comms_channel_memberships` membership for the current server-resolved DZN user ID. `/community` may later fetch the route only after the DZN Comms message-history UI integration approval preflight, with static fallback as default and failure path. Support remains static until a separate DZN Assist runtime approval. No message sending, runtime reactions, reports, moderation mutations, DZN Assist AI runtime, Durable Objects/WebSockets, analytics/tracking, Store/payment/live checkout changes, production mutations, retained exports, rankings, discovery, reviews, badges, seasons, events, Server Wars, CTF, XP, calling-card awards, public profile visibility, or competitive eligibility effects are approved |
| `/store` preview, `POST /api/store/orders`, `POST /api/store/orders/:orderId/checkout`, `POST /api/stripe/store-webhook`, `GET /api/account/purchases`, `GET /api/account/supporter-cards/[cardRef]/reveal`, and future `/wheel` surfaces | Read-only Store preview only; order creation and sandbox checkout require login and flags; Store webhook receipt requires Stripe signature and flags; private Account Purchases and private Supporter Card reveal require login, local/test flags, and read/reveal flags | Same read-only Store preview; authenticated local/test sandbox order creation may write pending player/account orders only when flags are explicitly enabled; authenticated sandbox checkout may create a test-mode Stripe Checkout Session only from an owned draft order and update `store_orders` to `checkout_created`; signed Store webhook receipt may record sanitized test-mode `store_payment_events` rows only; `GET /api/account/purchases` may read only that player's sanitized Store ledgers when explicitly enabled; `GET /api/account/supporter-cards/[cardRef]/reveal` may privately reveal only that player's own active/hidden Supporter Card serial/status after ownership proof when explicitly enabled; future guaranteed account-bound store purchases and earned-only wheel actions require later flags | Same player/account Store preview, order, sandbox checkout, receipt-only webhook, private Account Purchases, and private Supporter Card reveal boundary; owner entitlement adds no store advantage | Same player/account Store preview, order, sandbox checkout, receipt-only webhook, private Account Purchases, and private Supporter Card reveal boundary; Pro adds no store advantage | Catalog draft model adds only inactive `store_products` and `store_prices` plus local admin validation. `/store` is now a disabled-by-default read-only preview using safe catalog metadata and Supporter Card preview copy only. The sandbox order ledger schema adds private local/sandbox `store_orders`, `store_order_items`, and `store_payment_events` tables with `livemode = 0`, one-item order snapshots, unique provider-event ids, sanitized summaries, and no-fulfilment blockers. DZN Store Sandbox Order Creation Route Approval Slice: `POST /api/store/orders` adds pending local/test order ledger writes only behind session auth, `DZN_STORE_SANDBOX_RUNTIME=local` or `test`, Store sandbox flags, live-checkout blockers, and catalog safety checks; checkout remains unavailable in the order response. DZN Store Sandbox Checkout Session Approval Slice: `POST /api/store/orders/:orderId/checkout` adds test-mode Stripe Checkout Session only after an owned draft local/test order exists, with a server-controlled test Price binding, order-derived idempotency, safe redirects, `session_id: null`, and a `store_orders` status update only. Success redirects still do not fulfil purchases. DZN Store Sandbox Webhook Event Ledger Receipt Slice: `POST /api/stripe/store-webhook` verifies Stripe signatures against the raw body, accepts test-mode events only, and records sanitized `store_payment_events` receipt rows only. DZN Store webhook fulfilment approval preflight defines future verified test-mode fulfilment, eligible Checkout Session events, order-status transitions, exactly-once account-entitlement and Supporter Card boundaries, refund/chargeback rollback, schema preconditions, and proof requirements only; it leaves the webhook receipt-only and leaves `store_payment_events` blockers fixed to `0`. DZN Store Fulfilment Ledger Schema Migration Slice: `migrations/0073_dzn_store_fulfilment_ledger_schema.sql` adds local/test-only private schema for `account_entitlements`, `supporter_cards`, `store_fulfilment_attempts`, order/entitlement status history, and refund/dispute audit with `livemode = 0`; it does not add runtime fulfilment, Supporter Card issuance, earned spins, reward wheel runtime, production D1 apply, live checkout, or issue #49 change. DZN Store fulfilment runtime implementation approval preflight defines the future disabled-by-default local/test runtime sequence, exact write scope, idempotency, order transitions, account-entitlement creation, optional Supporter Card issuance, refund/chargeback rollback, proof matrix, and rollback path only; it adds no runtime writes, live checkout, production D1 apply, Stripe mutation, Cloudflare config mutation, or issue #49 change. DZN Store Account Purchases Read-Model Implementation Slice: `GET /api/account/purchases` is disabled by default, authenticated, private/no-store, current-user scoped, sanitized, local/test only, and read-only; it exposes Store purchase, entitlement, and private Supporter Card status only, while the v2 read model can advertise private reveal availability but still never returns card serials or generated art. DZN Store Supporter Card Private Reveal Implementation Slice: `GET /api/account/supporter-cards/[cardRef]/reveal` is disabled by default, authenticated, private/no-store, local/test only, and read-only; it reveals serial/status only after current-user ownership proof across Store order, order item, entitlement, and Supporter Card ledgers, and still adds no public reveal, generated card art, sharing, screenshot/export, notifications, migrations, live checkout, production D1 apply, Stripe mutation, Cloudflare config mutation, earned-spin ledger, reward wheel runtime, or issue #49 change. Future store uses verified webhook fulfilment, immutable payment ledgers, account-bound cosmetic entitlements, refund/chargeback revocation, admin price controls, earned-only spin ledger, three-spins-per-24-hour cap, four-hour cooldown, no buyable spins, no owner setup entitlement, no ranking/discovery/review/event/Server Wars/CTF/XP/calling-card award or competitive effect, and no live checkout or issue #49 changes until separately approved |
...
`/community` may later fetch the approved message-read route only when `NEXT_PUBLIC_DZN_COMMS_MESSAGE_HISTORY_UI_ENABLED=true` and the server read flags are also explicitly enabled. The route remains:

```text
GET /api/dzn-comms/channels/:channelId/messages
```

The approved channel mapping is `global`, `new_players`, `server_owners`, `events`, and `pandora_squad`. Public channel history remains free for logged-in Discord players when local/test flags are enabled. Private group history requires active trusted DZN user ID bridge membership on every request. Owner entitlement alone is not membership, and Starter/Pro do not change read access, ordering, page size, fallback behavior, author display, reaction availability, retention, or competitive eligibility.

The support surface remains static and does not fetch message history. DZN Assist stays blocked until a separate runtime approval defines source policy, privacy, cost controls, logging, refusal behavior, and rollback.

The UI must fall back to the static shell for disabled flags, loading failure, invalid cursor, unauthenticated public fallback, private group denial, unavailable route, timeout, or malformed response. Future message-history reads must be same-origin, no-store, bounded by an AbortController timeout, and stored only in active page state.
