...
- `POST /api/servers/[serverId]/adm/bulk-import` powers **Import ADM Files Now**. Preview can parse multiple files in one request, but real imports are sent by the dashboard one file at a time so a single Cloudflare Worker invocation does not parse/write/cache/queue an entire batch.
- Each sequential file import uses the same ADM parser/write path as scheduled processing. It writes kills, player events, PlayerList snapshots, raw events, stats, public cache, and plan-allowed Discord queues for that file, then the dashboard aggregates totals across the batch.
- If a file fails during a bulk import, successful file results remain visible and the failed file gets a **Retry this file** action. Retrying a whole or partial batch is safe because event dedupe prevents already imported ADM rows from double-counting stats.
