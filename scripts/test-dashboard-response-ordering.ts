...
assert.equal(dashboard.includes("const LIVE_STATS_POLL_INTERVAL_MS = 15000"), true, "Live stats must poll every 15 seconds.");
assert.equal(dashboard.includes("const DASHBOARD_HEALTH_POLL_INTERVAL_MS = 60000"), true, "Heavy dashboard health must poll every 60 seconds.");
...
assert.equal(dashboard.includes("void refreshDashboardLiveStats();\n          void refreshDashboardHealth();"), true, "Completed ADM jobs must trigger live and heavy refreshes.");
