...
export const onRequestPost: PagesFunction = async ({ request, env }) => {
  ...
  try {
    ...
    const result = await manualAwardProtectedBadge(env, {
      serverId: requireString(body.serverId),
      badgeCode: requireString(body.badgeCode),
      reason: typeof body.reason === "string" ? body.reason : null,
      awardType: typeof body.awardType === "string" ? body.awardType : "manual",
      isActive: typeof body.isActive === "boolean" ? body.isActive : true,
      actor: { userId: auth.user.id, role: auth.role },
    });
    return json(result);
  } catch (error) {
    return badgeEndpointError(error, "Badge award is temporarily unavailable.");
  }
};
...
function requireString(value: unknown) {
  ...
  if (!next) throw new BadgeEvaluationError("VALIDATION_FAILED", "serverId and badgeCode are required.", 400);
  return next;
}
...
function badgeEndpointError(error: unknown, fallbackMessage: string) {
  ...
  console.warn("DZN badge award endpoint failed", { requestId, error: error instanceof Error ? error.message : "unknown" });
  return json({
    ok: false,
    error: "BADGE_AWARD_UNAVAILABLE",
    errorCode: "BADGE_AWARD_UNAVAILABLE",
